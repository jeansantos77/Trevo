...
@Component({
  ...
  styleUrl: './list-combustivel.component.scss'
})


export class ListCombustivelComponent implements OnInit, AfterViewInit {
  _liveAnnouncer = inject(LiveAnnouncer);
  dialog = inject(MatDialog);
  toastr = inject(ToastrService);
  combustivelService = inject(CombustivelService);
  router = inject(Router);

  formName: string = "Combustível";
  buttonTooltip: string = "Cria um novo " + this.formName;
  entityPage: string = "/combustivel-form";

  displayedColumns: string[] = ['id', 'descricao', 'action'];
  dataSource = new MatTableDataSource<ICombustivel>();
  ...
  loadData() {
    this.combustivelService.getAll().subscribe(data => {
