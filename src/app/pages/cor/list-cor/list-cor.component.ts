...
@Component({
  ...
  templateUrl: './list-cor.component.html',
  styleUrl: './list-cor.component.scss'
})


export class ListCorComponent implements OnInit, AfterViewInit {
  _liveAnnouncer = inject(LiveAnnouncer);
  dialog = inject(MatDialog);
  toastr = inject(ToastrService);
  corService = inject(CorService);
  router = inject(Router);

  formName: string = "Cor";
  buttonTooltip: string = "Cria um novo " + this.formName;
  entityPage: string = "/cor-form";

  displayedColumns: string[] = ['id', 'descricao', 'action'];
  dataSource = new MatTableDataSource<ICor>();
