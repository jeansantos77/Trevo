...
@Component({
  ...
  templateUrl: './list-cambio.component.html',
  styleUrl: './list-cambio.component.scss'
})


export class ListCambioComponent implements OnInit, AfterViewInit {
  _liveAnnouncer = inject(LiveAnnouncer);
  dialog = inject(MatDialog);
  toastr = inject(ToastrService);
  CambioService = inject(CambioService);
  router = inject(Router);

  formName: string = "Câmbio";
  buttonTooltip: string = "Cria um novo " + this.formName;
  entityPage: string = "/cambio-form";

  displayedColumns: string[] = ['id', 'descricao', 'action'];
  dataSource = new MatTableDataSource<ICambio>();
  ...
  loadData() {
    this.CambioService.getAll().subscribe(data => {
  ...
  openDeleteConfirmation(id: number): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.CambioService.delete(id).subscribe(() => {
