...
@Component({
  ...
  templateUrl: './list-sit-veiculo.component.html',
  styleUrl: './list-sit-veiculo.component.scss'
})


export class ListSitVeiculoComponent implements OnInit, AfterViewInit {
  _liveAnnouncer = inject(LiveAnnouncer);
  dialog = inject(MatDialog);
  toastr = inject(ToastrService);
  situacaoVeiculoService = inject(SituacaoVeiculoService);
  router = inject(Router);

  formName: string = "Situação Veículo";
  buttonTooltip: string = "Cria um novo " + this.formName;
  entityPage: string = "/sit-veiculo-form";

  displayedColumns: string[] = ['id', 'descricao', 'action'];
  dataSource = new MatTableDataSource<ISituacaoVeiculo>();
  ...
  loadData() {
    this.situacaoVeiculoService.getAll().subscribe(data => {
  ...
  openDeleteConfirmation(id: number): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        this.situacaoVeiculoService.delete(id).subscribe(() => {
