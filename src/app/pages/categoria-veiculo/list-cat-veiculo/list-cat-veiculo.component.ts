...
@Component({
  ...
  templateUrl: './list-cat-veiculo.component.html',
  styleUrl: './list-cat-veiculo.component.scss'
})


export class ListCatVeiculoComponent implements OnInit, AfterViewInit {
  _liveAnnouncer = inject(LiveAnnouncer);
  dialog = inject(MatDialog);
  toastr = inject(ToastrService);
  categoriaVeiculoService = inject(CategoriaVeiculoService);
  router = inject(Router);

  formName: string = "Categoria Veículo";
  buttonTooltip: string = "Cria um novo " + this.formName;
  entityPage: string = "/cat-veiculo-form";

  displayedColumns: string[] = ['id', 'descricao', 'action'];
  dataSource = new MatTableDataSource<ICategoriaVeiculo>();
  ...
  loadData() {
    this.categoriaVeiculoService.getAll().subscribe(data => {
